...
const AdminAI = () => {
    ...
    useEffect(() => {
        if (!loading && reports.length > 0 && !summary && summaryRange === 'daily') {
    ...
    const createNewSession = async () => {
        try {
            ...
            setChatHistory([]);
            // Optionally update URL but not strictly required if state is managed
        } catch (error) {
                                ...
                                <form onSubmit={handleSend} className="p-4 bg-white border-t border-stone-100">
                                    <div className="relative flex gap-2">
                                        <input
                                            ...
                                            className="w-full bg-stone-50 border border-stone-200 rounded-xl px-5 py-4 pr-14 focus:outline-none focus:border-[#8ED462] focus:bg-white focus:ring-4 focus:ring-[#8ED462]/10 transition-all shadow-sm"
                                        />
                                        <button
                                            type="submit"
